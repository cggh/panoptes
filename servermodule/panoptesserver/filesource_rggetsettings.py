...
def response(returndata):
    ...
    try:
        if not os.path.exists(settingsFile):
            returndata['content'] = base64.b64encode_var2('AnnotMaxViewPortSize: 750000  # Maximum viewport (in bp) the genome browser can have in order to show the annotation track\nRefSequenceSumm: No          # Include a summary track displaying the reference sequence\n')
        else:
