...
function adaptMarker(payload) {
  ...
  let {props, key, markersCount} = payload;
  ...
  if (props.isHighlighted || markersCount === 1) {
    ...
    // NB: L.divIcon({html: null}) is a workaround to hide the default icon.
...
let LayeredMapMarkerLayer = React.createClass({
  ...
  render() {
    ...
    let {markers, color, children} = this.props;
...
console.log('MarkerLayer processing children');
      ...
      for (let i = 0, len = children.length; i < len; i++) {
        let adaptedChild = adaptMarker({props: children[i].props, key: i, markersCount: len});
...
console.log('MarkerLayer processing markers');
      ...
      for (let i = 0, len = markers.length; i < len; i++) {
        let adaptedMarker = adaptMarker({props: markers[i], key: i, markersCount: len});
...
console.log('MarkerLayer mapMarkers: %o', mapMarkers);
    ...
    return <FeatureGroup color={color}>{mapMarkers}</FeatureGroup>;
