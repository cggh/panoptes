...
export default function get(options, cancellation=null) {
  assertRequired(options,
    ['database', 'table', 'columns', 'query', 'regionField', 'regionStart', 'regionEnd', 'blockLimit']);
  const {database, table, columns, query, regionField, regionStart, regionEnd, blockLimit} = options;
  if (regionEnd < regionStart) {
    throw Error('PropertyRegionCache, end must be >= start');
  }
  const cacheKey = JSON.stringify({database, table, columns, query, regionField, blockLimit});
  //Find the 2 blocks that encapsulate us
  const blockLevel = Math.ceil(Math.log(regionEnd - regionStart) / Math.log(2));
  const blockIndex = Math.floor(regionStart / Math.pow(2.0, blockLevel));
